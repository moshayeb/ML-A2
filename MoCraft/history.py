...
def get_recent_context() -> str:
    ...
    lines = [f"{m.get('role', m.get('agent_name', 'unknown'))}: {m['content']}" for m in recent]
    return "\n".join(lines)
...
def compact_if_needed():
    ...
    text = "\n".join(
        f"{m.get('role', m.get('agent_name', 'unknown'))}: {m['content']}" for m in to_summarise
    )
    ...
    print(f"[COMPACT] Condensed {len(to_summarise)} messages → 1 summary. History: {len(new_history)} entries.")
    try:
...
def _summarise_with_claude(text: str, msg_count: int):
    try:
        ...
        _claude.update_tokens(usage.get("input_tokens", 0), usage.get("output_tokens", 0))
